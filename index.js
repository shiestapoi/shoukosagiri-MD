...
const { default: shiestaConnect,
    ...
    downloadHistory,
    getMessage,
    generateWAMessageContent } = require("@adiwajshing/baileys")
...
async function startHisoka() {
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            if (!global.owner.includes(m.sender.split`@`[0])) return
    ...
    hisoka.ev.on('connection.update', async (update) => {
        ...
        if (connection === "close") {
            ...
            const nomorBot = hisoka.user.id.split(':')[0] + '@s.whatsapp.net';
            console.log(chalk.green('✅ Tersambung dengan nomor: ' + nomorBot));
        }
